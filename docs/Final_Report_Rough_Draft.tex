...
Bao \textsc{Nguyen} \quad Fearghus \textsc{Tyler} \quad Yaqoub \textsc{Rabiah} \quad Fox \textsc{Kang}\\[0.45cm]
\normalsize \emph{Industry Sponsor:}\ MME / Ichor Systems --- Bridget Lannigan\\[0.12cm]
\emph{Faculty Advisors:}\ Prof. Andrew Greenberg \textbullet\ Dr. Jonathan Bird (EE) \textbullet\ Robert Paxton (ME)\\[0.45cm]
{\large \today}
\end{minipage}\\[0.5cm]
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \texttt{430.6}: motor full-load current taken from the nameplate / NEC tables (GE 5KE182BC205B test motor, FLA 7.6 A at 230 V) as the basis for all downstream sizing; final-motor sizing will be re-verified once the production motor is selected.
...
Programmed parameters on the Huanyang HY02D211B-T (as-built). Motor values
