...
\begin{itemize}
    \item Operator inputs (deadman buttons, E-stop, reset, lid interlock) $\rightarrow$ PLC discrete inputs.
    \item PLC outputs $\rightarrow$ VFD FOR/REV/RST inputs.
    \item VFD over-torque fault relay (FA/FB/FC) $\rightarrow$ PLC X5 input (fail-safe NC wiring).
...
\question{The over-torque relay function (\texttt{PD052 = 12}) did not fire the relay in our specific Huanyang revision. We worked around this with \texttt{PD052 = 02} (fault indication) and \texttt{PD123 = 3} (over-torque triggers self-trip, which fires the fault relay). Should we flag this as a vendor-specific limitation in the report, or just document the working configuration?}
...
\begin{itemize}
    \item VFD's FA/FB/FC relay programmed for over-torque/fault indication.
    \item Wired as fail-safe NC pair (FB + FC) per NFPA 79 convention for safety-relevant signals.
    \item PLC sees X5 = HIGH during normal operation, X5 = LOW when fault detected OR signal chain failure (wire break, dead VFD, etc.).
    \item 500 mA fast-blow fuse protects the +24V signal feed.
...
\begin{itemize}
    ...
    \item \textbf{RUNNING} $\rightarrow$ JAM\_DETECTED on falling edge of X5.
...
\begin{itemize}
    \item Counter increments on each X5 falling edge (rising edge of jam condition).
...
\begin{itemize}
    ...
    \item Prevents nuisance lockout during VFD initialization (PLC boots faster than VFD).
\end{itemize}
...
\begin{itemize}
    \item Wire break in jam signal: PLC sees X5 = LOW, treats as jam, eventually locks out. Safe failure.
